...
class Admin
{
    ...
    public function delete_user($api_key, $password, $email, $user_id)
    {
        if ($this->authentication->api_key_is_admin($api_key)) {
            ...
            $execute_prepared_statement = pg_execute($this->dbconn, 'admin_delete_user_get_admin', [$api_key]);
    ...
    public function password_reset_all_migration($api_key, $password)
    {
        if ($this->authentication->api_key_is_admin($api_key)) {
            ...
            $execute_prepared_statement = pg_execute($this->dbconn, 'get_user_pass_reset_migration', [$api_key]);
    ...
    public function verify_all_emails_migration($api_key, $password)
    {
        if ($this->authentication->api_key_is_admin($api_key)) {
            ...
            $execute_prepared_statement = pg_execute($this->dbconn, 'get_user_verify_all_emails_migration', [$api_key]);
    ...
    public function verify_user_emails($api_key, $password, $email)
    {
        if ($this->authentication->api_key_is_admin($api_key)) {
            ...
            if ($this->authentication->validate_password($user['user_id'], $password)) {
                ...
                $execute_prepared_statement = pg_execute($this->dbconn, 'verify_user_force', [$email]);
