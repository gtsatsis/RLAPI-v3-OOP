    ...
    public function create_user(Request $request)
    {
        ...
        if ($request->request->has('username') && $request->request->has('password') && $request->request->has('email')) {
            if ($request->request->has('promo_code')) {
                $createUser = $users->create_user($request->request->get('username'), $request->request->get('password'), $request->request->get('email'), ['promo_code' => $request->request->get('promo_code')]);
    ...
    public function set_user_tier(Request $request, $id)
    {
        ...
        if ($auth->isValidUUID($id)) {
            if ($request->request->has('tier') && $request->request->has('api_key')) {
                ...
                return new Response(json_encode(['success' => false, 'error_code' => 1082]));
            }
        } else {
            return new Response(json_encode(['success' => false, 'error_code' => 1083]));
        }
    }
